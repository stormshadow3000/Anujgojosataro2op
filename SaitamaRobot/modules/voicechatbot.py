...
from SaitamaRobot import pbot as LYCIA
...
async def Eren(_, message):
    if len(message.command) < 2:
        await message.reply_text("Founding Titan AI Voice Chatbot")
        return
    text = message.text.split(None, 1)[1]
    lycia = text.replace(" ", "%20")
    m = await message.reply_text("FoundingTitanRobot Is The Best...")
    try:
        L = await fetch(
